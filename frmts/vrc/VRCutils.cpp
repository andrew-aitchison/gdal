...
{
    return static_cast<int32_t>(VRGetUInt(base, byteOffset));
}
...
int32_t VRReadInt(VSILFILE *fp)
{
    ...
    // if (ret<4) return EOF;
    return static_cast<int32_t>(VRGetUInt(&buf, 0));
}
...
uint32_t VRReadUInt(VSILFILE *fp)
{
    ...
    vv |= buf[0];
    vv |= static_cast<unsigned int>(buf[1]) << 8U;
    vv |= static_cast<unsigned int>(buf[2]) << 16;
                                           ...
                                           const char *szCountry)
{
    ...
    switch (nCountry)
    {
        ...
        case 17:
            ...
            if (szCountry && ((szCountry[0] == 'e' && szCountry[1] == 's') ||
